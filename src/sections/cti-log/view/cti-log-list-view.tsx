...
import { useGetCtiLogs } from 'src/actions/cti-log';
import { DashboardContent } from 'src/layouts/dashboard';

import { EmptyContent } from 'src/components/empty-content';
import { CustomBreadcrumbs } from 'src/components/custom-breadcrumbs';

import { defaultCtiLogFilters } from 'src/types/cti-log';

import { paths } from '../../../routes/paths';
import { CtiLogFilter } from '../cti-log-filter';
import { useRouter } from '../../../routes/hooks';
import { defaultPagination } from '../../../utils/pagination';
import { DnaPagination } from '../../../components/dna-pagination';

export function CtiLogListView() {
  const router = useRouter();

  const [sortModel, setSortModel] = useState<GridSortModel>([]);
  const [pagination, setPagination] = useState<GridPaginationModel>(defaultPagination);
  const [filters, setFilters] = useState<CtiLogFilters>(defaultCtiLogFilters);
  const { data, loading, totalCount } = useGetCtiLogs(pagination, sortModel, filters);
  const [tableData, setTableData] = useState<CtiLog[]>([]);
  ...
  return (
    <DashboardContent className="dna-common-list">
      ...
      <Card className="list-filter" sx={{ mb: { xs: 3, md: 5 } }}>
        <CtiLogFilter onSearch={(f) => setFilters(f)} />
      </Card>
      <Card
        className="list-grid"
        sx={{
